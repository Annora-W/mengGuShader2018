...
void Shader::Init(const char * vs, const char * fs) {
	...
	if (mProgram != 0) {
		mModeMatrixLocation = glGetUniformLocation(mProgram, "ModelMatrix");
...
void Shader::Bind(float * M, float * V, float * P) {
	...
	if (mModeMatrixLocation >= 0)
		glUniformMatrix4fv(mModeMatrixLocation, 1, GL_FALSE, M);
...
GLuint Shader::SetTexture(const char * name, GLint texture) {
	auto iter = mUniformTextures.find(name);
	GLint oldTexture = 0;
	if (iter == mUniformTextures.end()) {
		GLint location = glGetUniformLocation(mProgram, name);
